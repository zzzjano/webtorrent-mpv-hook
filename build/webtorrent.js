...
var overlayText = '';
var playlist = [];
var fileIndex = -1; // Przechowuje indeks pliku, który ma być załadowany
...
function openPlaylist() {
    if (fileIndex >= 0 && fileIndex < playlist.length) {
        ...
        var fileToLoad = playlist[fileIndex];
        if (fileToLoad) {
...
function onLoadHook() {
    ...
    try {
        ...
        var fileIndexMatch = url.match(/#fileIndex=(\d+)/);
        if (fileIndexMatch && fileIndexMatch[1]) {
            ...
            var cleanUrl = url.replace(/#fileIndex=\d+/, '');
